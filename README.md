<p align="center">
  <img src="assets/system-map.svg" alt="Detailed system map for GitHub Authentication" width="100%">
</p>
...
Tool integrations can create accidental side effects when authentication, scope, preview, execution, and verification are not separated. **GitHub Authentication** turns that work into an explicit sequence with visible inputs, outputs, review points, and completion evidence.

## Why the repository has this name

The shared `hermes-skill-` prefix identifies this as a portable Hermes workflow package. `github-auth` names the capability directly—github auth—so the repository remains searchable and understandable outside the original AI-OS workspace. The public title is **GitHub Authentication**.
...
| What does it do? | Set up GitHub authentication using HTTPS tokens, SSH keys, or gh CLI login without exposing credentials. |
...
![GitHub Authentication system map](assets/system-map.svg)

## Operation sequence

![GitHub Authentication actor and data sequence](assets/operation-sequence.svg)
...
![Illustrative output produced by GitHub Authentication](assets/example-output.svg)
...
> In a disposable test workspace, set up GitHub authentication using HTTPS tokens, SSH keys, or gh CLI login without exposing credentials. Return the result, the evidence used to verify it, and any limitations or actions that still require approval.
